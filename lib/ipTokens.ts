...
// IP Token contract address on Story Testnet (Aeneid)
// TODO: Replace with actual IP token address from Story Protocol
export const IP_TOKEN_ADDRESS = process.env.NEXT_PUBLIC_IP_TOKEN_ADDRESS as `0x${string}` || '0x0000000000000000000000000000000000000000';
...
/**
 * Hook to get native Story token balance (for gas)
 */
export function useStoryBalance() {
  const { address } = useAccount();
